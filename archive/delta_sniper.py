...
# Cooldown period in seconds (30 seconds - alerts ASAP while preventing spam)
COOLDOWN_SECONDS = 30
...
# Enable Google Search Grounding (requires GOOGLE_API_KEY env var)
ENABLE_GOOGLE_SEARCH = True

# Threshold for triggering Google Search context (15% = 0.15)
...
def send_alert(ticker: str, contract_name: str, old_price: float, new_price: float, change_pct: float, context: Optional[Dict[str, str]] = None):
    """
    ...
    Args:
        ...
        context: Optional dictionary with 'summary' and 'url' from Google Search
    """
    ...
    if context and context.get('summary'):
        ...
        if context.get('url') and context['url'] != 'No URL found':
    ...
    try:
        ...
        if last_price > 0:
            ...
            if change_pct >= SPIKE_THRESHOLD:
                # Get AI context if spike is >= AI_CONTEXT_THRESHOLD (15%)
                context = None
                if change_pct >= AI_CONTEXT_THRESHOLD and ENABLE_GOOGLE_SEARCH:
                    print(f"🔍 Fetching market context for {contract_name}...")
                    context = get_market_context(ticker, contract_name, enabled=ENABLE_GOOGLE_SEARCH)
                    if context:
                        print(f"✓ Context retrieved: {context.get('summary', '')[:100]}...")
                    else:
                        print("⚠️  Could not fetch context")
                
                # Trigger alert with contract name and context
                send_alert(ticker, contract_name, last_price, current_price, change_pct, context)
