...
Monitors ORDERBOOK DELTAS (all order changes, not just trades).
WARNING: May produce false alerts from lowball bids/asks.
"""
...
# Database path
DB_PATH = "kalshi_monitor_orderbook.db"  # Separate DB to avoid conflicts
        ...
        if msg_type == "trade":
            ...
            if ticker and price is not None:
                ...
                if old_price is not None and old_price > 0:
                    ...
                    print(f"[CACHE] {ticker}: ${price:.2f} (initial)")
                
                update_cached_price(ticker, price)
        
        # Handle orderbook updates (WARNING: may include lowball bids/asks)
        elif msg_type == "orderbook_snapshot" or msg_type == "orderbook_delta":
            ticker = data.get("msg", {}).get("market_ticker")
            price_str = data.get("msg", {}).get("price_dollars")
            
            if ticker and price_str:
                try:
                    price = float(price_str)
                    
                    print(f"[ORDERBOOK] {ticker}: ${price:.2f}")
                    ...
                    if old_price is not None and old_price > 0:
                        ...
                        if abs(change_pct) >= threshold and not is_in_cooldown(ticker):
                            ...
                            print(f"[UPDATE] {ticker}: ${old_price:.2f} → ${price:.2f} ({change_pct:+.2%})")
                    else:
                        print(f"[CACHE] {ticker}: ${price:.2f} (initial)")
                    
                    update_cached_price(ticker, price)
                except (ValueError, TypeError):
                    pass  # Skip invalid price data
...
def on_open(ws):
    ...
    for event_ticker in TARGET_MARKETS:
        try:
            ...
            # Subscribe to each contract (orderbook_delta channel)
            for market in markets:
                ...
                if ticker:
                    subscribe_msg = {
                        ...
                        "cmd": "subscribe",
                        "params": {
                            "channels": ["orderbook_delta"],  # Track all orderbook changes
    ...
    print(f"\n🎯 Monitoring {total_contracts} contracts (ORDERBOOK MODE)")
    ...
    print(f"   ⚠️  WARNING: Orderbook mode may produce false alerts from lowball bids/asks\n")
...
def main():
    ...
    print("="*60)
    print("Kalshi WebSocket Monitor - Orderbook Sniper")
    print("="*60)
